...
function PaymentPollingPage() {
  ...
  const navigate = useNavigate({ from: '/donate/$creatorId/payment/$donationId' })
  ...
  useEffect(() => {
    const fetchDonation = async () => {
      try {
        const res = await fetch(`${API_URL}/public/donations/${donationId}`);
  ...
  if (loading) return <div className="min-h-screen bg-background flex items-center justify-center text-white"><Loader2 className="animate-spin text-blue-500 w-12 h-12" /></div>
      ...
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
        <div className="w-full max-w-md bg-surface/80 border border-green-500/30 rounded-2xl p-8 backdrop-blur-xl text-center space-y-6">
          ...
          <div>
            ...
            <p className="text-text-muted">Thank you for supporting {pageInfo?.creator?.displayName || creatorId}</p>
          </div>
      ...
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
        <div className="w-full max-w-md bg-surface/80 border border-red-500/30 rounded-2xl p-8 backdrop-blur-xl text-center space-y-6">
          ...
          <div>
            ...
            <p className="text-text-muted">There was an issue processing your payment.</p>
          </div>
          <Link 
            to="/donate/$creatorId"
            params={{ creatorId }}
          ...
          <div className="bg-surface/80 backdrop-blur-xl rounded-2xl border border-white/10 overflow-hidden shadow-2xl">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex justify-between items-center py-2 border-b border-white/5">
                ...
                <span className="text-white font-medium">{paymentInstructions.bank}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-white/5">
                <span className="text-text-muted">Account</span>
                <div className="text-right">
                  <div className="text-white font-medium">{paymentInstructions.accountNumber}</div>
                  <div className="text-xs text-text-muted uppercase">{paymentInstructions.accountName}</div>
                </div>
              </div>
              <div className="flex flex-col py-2 border-b border-white/5">
                ...
                <div className="flex items-center justify-between bg-black/30 rounded-lg p-3 border border-white/5">
                  <span className="text-white font-mono text-lg font-bold tracking-wider">{paymentInstructions.content}</span>
                  <button className="text-blue-400 hover:text-blue-300 transition-colors p-2" onClick={() => navigator.clipboard.writeText(paymentInstructions.content)}>
                    <Copy size={18} />
                  </button>
                </div>
              </div>
  ...
  return <div className="min-h-screen bg-background flex items-center justify-center text-white"><Loader2 className="animate-spin text-blue-500 w-12 h-12" /></div>
}
