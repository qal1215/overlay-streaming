...
function PublicDonationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate({
        to: `/donate/$creatorId/payment/$donationId`,
        params: { creatorId, donationId: data.id },
      });
    } catch (err: any) {
    ...
    <div className="min-h-screen w-full bg-background py-12 px-4 relative overflow-y-auto">
      ...
      <div className="max-w-md mx-auto relative z-10">
          ...
          <div className="w-24 h-24 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 mx-auto mb-4 p-1 shadow-xl shadow-purple-500/20">
            <div className="w-full h-full rounded-full bg-surface overflow-hidden flex items-center justify-center">
              {pageInfo.creator.avatarUrl ? (
                <img
                  src={pageInfo.creator.avatarUrl}
                  alt="Avatar"
                  className="w-full h-full object-cover"
                />
              ) : (
                <span className="text-4xl font-bold text-white">
                  {pageInfo.creator.displayName.charAt(0)}
                </span>
              )}
            </div>
          </div>
          <h1 className="text-3xl font-bold text-white">
            {pageInfo.creator.displayName}
          </h1>
        ...
        <form
          ...
          className="bg-surface/60 backdrop-blur-xl rounded-2xl border border-white/10 p-6 shadow-2xl space-y-6"
        >
          {error && (
          ...
          <div>
            ...
            <div className="relative">
              ...
              />
              <div className="absolute left-4 top-1/2 -translate-y-1/2 text-text-muted font-bold">
                ₫
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4 pt-4 border-t border-white/5">
            <div>
              ...
              <input
                ...
                placeholder={
                  pageInfo.donation.allowAnonymous ? "Anonymous" : "Your Name"
                }
